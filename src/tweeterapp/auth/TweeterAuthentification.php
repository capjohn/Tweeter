...
class TweeterAuthentification extends \mf\auth\Authentification {
    ...
    {
        $nomUser = \tweeterapp\model\User::where('username','=',$username)->first();
        if($nomUser != null)
        {
            //throw new Exception("Un compte possède déja ce nom d'utilisateur");
            echo ("Un Utilisateur a déja ce compte");
            ...
            $user->level = $level;
            $passhach= $this->hashPassword($pass);
            $user->password = $passhach;
            $user->followers = 0;
            $user->save();
        }
    }
